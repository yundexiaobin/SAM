...
export function useHook() {
  ...
  function initSearchConfig() {
    setTimeout(async () => {
      const response = await http.services.apiSysRoleSearchConfigGet({});
  ...
  function onSearch() {
    loading.value = true;
    setTimeout(async () => {
      const response = await http.services.apiSysRoleSearchPost({
  ...
  function handleDelete(row) {
    setTimeout(async () => {
      await http.services.apiSysRoleDelete(row.id);
  ...
  async function getDetailAsync(val?: number) {
    return await http.services.apiSysRoleGet(val);
  }
