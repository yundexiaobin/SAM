...
export function useHook() {
  ...
  function initSearchConfig() {
    setTimeout(async () => {
      const response = await http.services.apiStockPerceptionSearchConfigGet({});
  ...
  function handleDelete(row) {
    setTimeout(async () => {
      await http.services.apiStockPerceptionDelete(row.id);
  ...
  function onSubmit(data: any) {
    ...
    setTimeout(async () => {
      if (isUpdated) {
        await http.services.apiStockPerceptionPut(parseInt(value.id), value);
      } else {
        await http.services.apiStockPerceptionPost(value);
      }
  ...
  async function getDetailAsync(val?: number) {
    return await http.services.apiStockPerceptionGet(val);
  }
